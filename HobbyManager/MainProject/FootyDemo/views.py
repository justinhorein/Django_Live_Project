...
def api_response(request):
    ...
    if request.method == 'POST':  # This block will handle all post backs from the forms
        ...
        if 'league' in request.POST:  # Checks for a league item, knows we're in the third dropdown
            league_id = request.POST[
                'league']  # Gets the value of league from the dropdown menu, an element of the Post dictionary
            ...
            context.update({'leagues': leagues, 'childArea': request.POST[
                'childArea']})  # Adds information to context to populate dropdowns
...
def matches(request, code):
    ...
    #print(league_dictionary['matches'])
    for match in league_dictionary['matches']:  # Iterates through the matches within the JSON response
        local_date = convert_to_localtime(
            match['utcDate'])  # Gets utcDate and converts it to local time, function below
        ...
        context['matches'].append(
            game)  # Adds the game item to the array in the dictionary, then iterates through next item
    context.update({'local_tz': timezone.get_current_timezone_name()})  # Adds the local timezone to the context object

    return render(request, 'FootyDemo/footy_matches.html',
                  context)  # Sends completed dictionary of matches and timezone to template
...
def mls_news(request):
    ...
    soup = BS(source.content,
              'html.parser')  # Initial processing of the html by beautiful soup, soup is now a navigatable object
    ...
    for node in nodes:  # Iterates through all the objects with class of node-title
        ...
        timestamp = node.find_next_siblings(class_="timestamp")[
            0].get_text()  # Uses class and sibling relationship to get the timestamp
        url = "https://www.mlssoccer.com" + link  # Modifies the link to a full url, since the links were relative
        article = {'title': title, 'url': url,
                   'date': timestamp}  # Creates and article object dictionary with needed elements
...
def convert_to_localtime(utctime):
    localdatetime = datetime.datetime.strptime(utctime,
                                               '%Y-%m-%dT%H:%M:%SZ')  # Uses strptime to pull out time elements, this must match the format being sent
